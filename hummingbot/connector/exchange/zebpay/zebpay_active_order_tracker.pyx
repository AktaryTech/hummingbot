...
ZebpayOrderBookTrackingDictionary = Dict[Decimal, Dict[str, Dict[str, any]]]

cdef class ZebpayActiveOrderTracker:
    def __init__(self,
                 active_asks: ZebpayOrderBookTrackingDictionary = None,
                 active_bids: ZebpayOrderBookTrackingDictionary = None):
        super().__init__()
    ...
    @property
    def active_asks(self) -> ZebpayOrderBookTrackingDictionary:
        return self._active_asks

    @property
    def active_bids(self) -> ZebpayOrderBookTrackingDictionary:
    ...
    def get_rates_and_quantities(self, entry) -> tuple:
        # price, quantity
        return float(entry.get("price")), float(entry.get("amount"))
    ...
    cdef tuple c_convert_snapshot_message_to_np_arrays(self, object message):
        ...
        for snapshot_orders, active_orders in [(content["bids"], self._active_bids), (content["asks"], self.active_asks)]:
            for order in snapshot_orders:
                ...
                order_dict = {
                # timestamp used in lieu of sequence number
                    "order_id": timestamp,
                    "amount": amount
                }
