...
class ZebpayOrderBookTracker(OrderBookTracker):
    _zobt_logger: Optional[HummingbotLogger] = None

    @classmethod
    def logger(cls) -> HummingbotLogger:
        if cls._zobt_logger is None:
            cls._zobt_logger = logging.getLogger(__name__)
        return cls._zobt_logger

    def __init__(self,
                 trading_pairs: Optional[List[str]] = None,
                 domain: str = "com"):
        super().__init__(data_source=ZebpayAPIOrderBookDataSource(trading_pairs=trading_pairs),
        ...
        self._order_books: Dict[str, ZebpayOrderBook] = {}
        self._domain = domain
        self._saved_message_queues: Dict[str, Deque[OrderBookMessage]] = defaultdict(lambda: deque(maxlen=1000))
        self._active_order_trackers: Dict[str, ZebpayActiveOrderTracker] = defaultdict(ZebpayActiveOrderTracker)

    @property
    def exchange_name(self) -> str:
        """
        Provides the name of the current exchange
    ...
    async def _track_single_book(self, trading_pair: str):
        ...
        message_queue: asyncio.Queue = self._tracking_message_queues[trading_pair]
        order_book: ZebpayOrderBook = self._order_books[trading_pair]
        active_order_tracker: ZebpayActiveOrderTracker = self._active_order_trackers[trading_pair]
